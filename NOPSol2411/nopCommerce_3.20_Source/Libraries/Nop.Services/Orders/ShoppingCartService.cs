...
namespace Nop.Services.Orders
{
    ...
    public partial class ShoppingCartService : IShoppingCartService
    {
        ...
        public virtual IList<string> AddToCart(Customer customer, Product product,
            ...
            decimal customerEnteredPrice, int quantity, bool automaticallyAddRequiredProductsIfEnabled, string VarParentItemId)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");
            ...
            if (shoppingCartItem != null)
            {
                ...
                if (!String.IsNullOrEmpty(shoppingCartItem.ParentItemId))
                {
                    warnings.Add("This Replacement Part's Quantity cannot change from " + shoppingCartItem.Quantity);
                    return warnings;
                }
                else if (!String.IsNullOrEmpty(VarParentItemId))
                {
                    //in future, change this so new item no. will create a new line
                    warnings.Add("Cannot Add Replacement Parts as an identical part is already in the cart (Please remove the "+"original part"+" and add the replacement parts from your account)");
                    return warnings;
                }
            ...
            decimal customerEnteredPrice, int quantity, bool resetCheckoutData)
        {
            ...
            if (shoppingCartItem != null)
            {
                if (!String.IsNullOrEmpty(shoppingCartItem.ParentItemId))
                {
                    if (quantity > 1)
                    {
                        warnings.Add("Replacement Parts Quantity cannot exceed 1");
                        return warnings;
                    }
                }
                if (resetCheckoutData)
        ...
        public virtual void MigrateShoppingCart(Customer fromCustomer, Customer toCustomer, bool includeCouponCodes)
        {
            ...
            for (int i = 0; i < fromCart.Count; i++)
            {
                ...
                AddToCart(toCustomer, sci.Product, sci.ShoppingCartType, sci.StoreId, 
                    sci.AttributesXml, sci.CustomerEnteredPrice, sci.Quantity, false, sci.ParentItemId);
            }
            for (int i = 0; i < fromCart.Count; i++)
            {
                var sci = fromCart[i];
