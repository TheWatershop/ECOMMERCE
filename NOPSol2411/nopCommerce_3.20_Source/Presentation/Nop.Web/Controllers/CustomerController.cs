...
namespace Nop.Web.Controllers
{
    public partial class CustomerController : BaseNopController
    {
        ...
        public CustomerController(IAuthenticationService authenticationService,
            ...
            IOrderProcessingService orderProcessingService, IOrderService orderService, IComponentService componentService,
        ...
        [NonAction]
        protected CustomerComponentListModel PrepareCustomerComponentsListModel(Customer customer)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");

            var model = new CustomerComponentListModel();
            model.NavigationModel = GetCustomerNavigationModel(customer);
            model.NavigationModel.SelectedTab = CustomerNavigationEnum.Components;
            var Components = _componentService.SearchComponents(customerId: customer.Id);
            foreach (var Component in Components)
            {
                var ComponentModel = new CustomerComponentListModel.ComponentDetailsModel()
                {
                    Id = Component.Id,
                    ProductId = Component.ProductId,ExpiryDate = Component.ExpiryDate ?? DateTime.MinValue, ParentItemId = Component.ParentItemId, ProductName = Component.ProductName, Quantity = Component.Quantity,
                    ParentItemName = Component.ParentItemName,
                    ParentItemSerial = Component.ParentItemSerial,Location = Component.Location, Sub_Location = Component.Sub_Location
                    //CreatedOn = _dateTimeHelper.ConvertToUserTime(Component.CreatedOnUtc, DateTimeKind.Utc),
                    //OrderStatus = Component.OrderStatus.GetLocalizedEnum(_localizationService, _workContext),
                };
                //var orderTotalInCustomerCurrency = _currencyService.ConvertCurrency(order.OrderTotal, order.CurrencyRate);
                //orderModel.OrderTotal = _priceFormatter.FormatPrice(orderTotalInCustomerCurrency, true, order.CustomerCurrencyCode, false, _workContext.WorkingLanguage);
        ...
        public ActionResult Components()
        {
            ...
            var customer = _workContext.CurrentCustomer;
            var model = PrepareCustomerComponentsListModel(customer);
            return View(model);
        }
